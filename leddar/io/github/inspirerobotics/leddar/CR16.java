...
public class CR16 {
	...
	static int CRC16(byte[] bytes) {
		...
		if(bytes.length < 2) {
			throw new IllegalArgumentException("Buffer too small to add CRC16 Bytes...");
		}
	...
	public static boolean checkCRC16(byte[] bytes) {
		int calculated = CRC16(bytes);
		
		int val = ((bytes[bytes.length - 1] & 0xff) << 8) | (bytes[bytes.length - 2] & 0xff);
		
		return val == calculated;
	}
